...
export default function ProfilePage() {
  ...
  const getFirstContactInfo = (commList: Communication[], oldPhone: Communication, oldEmail: Communication) => {
  ...
  useEffect(() => {
    const fetchProfileAndImage = async () => {
      try {
        ...
        const result = getFirstContactInfo(profileData["Communication List"], phoneData, emailData);
  ...
  const refetchBookings = async () => {
    ...
    await sleep(1000);
    const bookings = await fetchData("bookings");
    const filteredBookings = bookings.filter((booking: { clientID: any }) => booking.clientID === params.clientId);
    closeBookingModal();
    setBookingData(filteredBookings);
  }
      ...
      if (phoneData) {
        if (isValidPhoneNumber(phoneData["Communication Detail"])) {
          ...
          setProfileData((prev: any) => ({
            ...prev,
            "Communication List": [
              ...prev["Communication List"].filter((comm: any) => comm["Communication Type"] !== "C"), // Remove existing phone
          ...
          setProfileData((prev: any) => ({
            ...prev,
            "Communication List": prev["Communication List"].filter((comm: any) => comm["Communication Type"] !== "C"), // Remove phone data
          }));
        }
      }
      ...
      if (emailData) {
        if (isValidEmail(emailData["Communication Detail"])) {
          ...
          setProfileData((prev: any) => ({
            ...prev,
            "Communication List": [
              ...prev["Communication List"].filter((comm: any) => comm["Communication Type"] !== "M"), // Remove existing email
          ...
          setProfileData((prev: any) => ({
            ...prev,
            "Communication List": prev["Communication List"].filter((comm: any) => comm["Communication Type"] !== "M"), // Remove email data
          }));
        }
      }
    }
        ...
        <Card className="md:col-span-2 p-5">
          ...
          <CardBody className="gap-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              />
              {!isValidEmail(emailData["Communication Detail"]) && isEditing && (
                <div className="text-gray-500 text-sm px-3 pt-1">Invalid email will not be saved.</div>
              )}
              </div>
              ...
              />
              {!isValidPhoneNumber(phoneData["Communication Detail"]) && isEditing && (
                <div className="text-gray-500 text-sm px-3 pt-1">Invalid phone number will not be saved.</div>
              )}</div>
