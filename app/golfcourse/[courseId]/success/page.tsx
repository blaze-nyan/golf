    ...
    <div>
      ...
      {showConfetti && (
        <Confetti
          ...
          confettiSource={{ x: 0, y: window.innerHeight, w: window.innerWidth, h: 0 }} // Start from the bottom left
        />
      )}
        ...
        <div className="p-4 space-y-2 text-sm text-gray-700">
          ...
          <SectionWrapper disabled={isSectionDisabled(0)}>
            <div className="flex gap-3 items-center">
              ...
              />
              <div>
                <div className="font-semibold">{bookingDetails.courseName || "Course Name"}</div>
                <div className="text-gray-500">{bookingDetails.courseLocation || "N/A"}</div>
              </div>
            </div>
          </SectionWrapper>
          ...
          <SectionWrapper disabled={isSectionDisabled(1)}>
            <InfoGrid>
              <InfoItem icon="mdi-golf" label="Type" value={bookingDetails.bookingType === 1 ? "9-Hole" : "18-Hole"} />
              <InfoItem icon="mdi-calendar" label="Date" value={dateToString(convertExcelDateToJSDate(bookingDetails.teeDate)) || "Select"} />
              <InfoItem icon="mdi-clock" label="Time" value={convertMinutesToTimeWithAMPM(bookingDetails.teeTime) || "Select"} />
            </InfoGrid>
          </SectionWrapper>
          ...
          <SectionWrapper disabled={isSectionDisabled(2)}>
            <InfoGrid>
              <InfoItem icon="mdi-golf" label="Golfers" value={bookingDetails.numberOfGolfers} />
              <InfoItem icon="mdi-person" label="Guests" value={bookingDetails.numberOfnonPlayers} />
              <InfoItem icon="mdi-backpack" label="Caddies" value={bookingDetails["Caddies"]} />
              <InfoItem icon="mdi-car" label="Carts" value={bookingDetails["Golf Cart"]} />
              <InfoItem icon="mdi-food" label="Food" value={bookingDetails["Food & Drinks"]} />
            </InfoGrid>
          </SectionWrapper>
          ...
          <SectionWrapper disabled={isSectionDisabled(3)}>
            <div className="flex items-center gap-2 font-semibold text-medium">
              <Icon icon="mdi-currency-btc" className="text-green-600 textlg" />
              <span>Total: {bookingDetails.price ? `${bookingDetails.price} THB` : "N/A"}</span>
            </div>
          </SectionWrapper>
          <Button
...
/** Section Wrapper */
const SectionWrapper = ({ children, disabled }: { children: React.ReactNode; disabled: boolean }) => (
  <div className={`p-3 border border-gray-200 rounded-lg ${disabled ? "opacity-50 pointer-events-none" : "bg-gray-50"}`}>
    {children}
  </div>
);
